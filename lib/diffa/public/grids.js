...
Diffa.HttpDataSource = (function() { 
    function HttpDataSource(url) {
        this._url = url;
    };

    HttpDataSource.prototype.whenUpdated = function whenUpdated(f) {
    };

    // Unit => Promise[Error, List[Trade]]
    HttpDataSource.prototype.retrieve = function retrieve() {
        return Q.when($.ajax(this._url, { dataType: 'json' }))
                .then(function(json) { 
                    return _.map(json, function(t) { return new Diffa.Trade(t); });
                });
    };
