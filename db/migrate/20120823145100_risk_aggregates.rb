class RiskAggregates < ActiveRecord::Migration
  def up
    execute %q{create view risks_view (id, trade_date, type_code, version, user) as select trade_id, trade_date, 'F', coalesce(version, ''), user_id from futures union select trade_id, trade_date, 'O', coalesce(version, ''), user_id from options order by id}
